...
<label>Number For Point Game  :</label>
<input type="text" name="point" id="point" required="required" placeholder="Number for Point Game"/><br/><br />
<label>Date :</label>
	 ...
	 <input name="second_date" type="text" size=10 maxlength=10  onkeyup="this.value=this.value.replace(/^(\d\d)(\d)$/g,'$1/$2').replace(/^(\d\d\/\d\d)(\d+)$/g,'$1/$2').replace(/[^\d\/]/g,'')"> <br>
...
$sql = "INSERT INTO second (point,second_date)
VALUES ('".$_POST["point"]."','".$_POST["second_date"]."')";
...
$sql = "SELECT * FROM second ORDER BY id DESC LIMIT 0,50";
...
$result = mysqli_query($conn, $sql);
    ...
    while($row = mysqli_fetch_assoc($result)) {
            ...
            <td><?php echo $row['second_date']; ?></td> 
					 <td ><a style="color:white;"href="delete_point.php? id=<?php echo $row['id']; ?>">Delete</a></td>
					 <td><a style="color:white;" href="update_point.php?id=<?php echo $row["id"]; ?>">Update</a></td>
